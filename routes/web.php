...
Route::middleware('auth')->group(function () {
    ...
    // 通知(3 ロール共通。admin は本 MVP では通知を受信しないため一覧は常に空)
    ...
    // コーチメモ(担当コーチ + 管理者のみ操作可、受講生は閲覧含め拒否。認可は EnrollmentNotePolicy に集約)
...
Route::middleware(['auth', 'role:student', 'active-learning'])->group(function () {
    ...
    // 個人学習目標(受講登録詳細画面に組み込み表示、単独の一覧 / 詳細画面は持たない)
...
Route::middleware(['auth', 'role:admin'])->prefix('admin')->group(function () {
    ...
    // 面談パック管理(追加面談購入用 SKU の CRUD + 状態遷移、admin のみ)
    ...
    // 受講プラン管理(受講期間 / 初期付与面談回数の CRUD + 状態遷移、admin のみ)
...
Route::middleware(['auth', 'role:student', 'active-learning'])->prefix('meeting-quota')->name('meeting-quota.')->group(function () {
    ...
    // 追加面談パックの購入(Stripe Checkout への委譲)
...
// ============================================================
// Stripe Webhook(認証なし、署名検証のみが正当性の担保。CSRF 対象外は VerifyCsrfToken::$except で除外)
// ============================================================
Route::post('webhooks/stripe', [StripeWebhookController::class, 'handle'])->name('webhooks.stripe');

// ============================================================
// 受講生専用ルート — AI 相談(Gemini チャットボット)
//
// config('ai-chat.enabled') が false の間はルート自体を登録しない
// (フローティングウィジェット・フル画面とも一切利用できなくなる)。
// ============================================================
