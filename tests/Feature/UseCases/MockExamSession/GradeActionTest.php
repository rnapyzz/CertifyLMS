...
class GradeActionTest extends TestCase
{
    ...
    public function test_grades_session_with_four_of_five_correct_matches_ticket_example(): void
    {
        // チケット記載の再現手順そのもの: 5 問中 4 問正解 = 80.00(合格基準点 60% を超えて合格)
        $mockExam = MockExam::factory()->published()->passingScore(60)->create();
        $questions = collect();
        for ($i = 0; $i < 5; $i++) {
            $questions->push(MockExamQuestion::factory()->forMockExam($mockExam)->withOptions(4, 0)->create(['order' => $i]));
    ...
    public function test_grades_session_with_all_correct_scores_100(): void
    {
        // チケット記載の再現手順: 全問正解(5/5)でも 100.00 になり合格すること
        $mockExam = MockExam::factory()->published()->passingScore(60)->create();
        $questions = collect();
        for ($i = 0; $i < 5; $i++) {
            $questions->push(MockExamQuestion::factory()->forMockExam($mockExam)->withOptions(4, 0)->create(['order' => $i]));
